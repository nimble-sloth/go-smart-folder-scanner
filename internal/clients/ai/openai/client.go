...
func (c *Client) Chat(ctx context.Context, msgs []ai.Message, opts ai.ChatOptions) (string, error) {
    if c.key == "" { return "", errors.New("OPENAI_API_KEY not set") }
    payload := struct {
    ...
    if resp.StatusCode/100 != 2 { return "", errors.New("openai: non-2xx response") }

    var out struct {
        Choices []struct{ Message ai.Message `json:"message"` } `json:"choices"`
    }
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return "", err }
    if len(out.Choices) == 0 { return "", errors.New("openai: empty choices") }
